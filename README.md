...
4. **拟合椭圆轴比**：`cv2.fitEllipse()` 获取长短轴之比，对倾斜椭圆比外接矩形长宽比更鲁棒
5. **形状判断**：
   - 3个顶点 → 三角形
   - 4个顶点 → 正方形（长宽比<1.2）/ 矩形
   - 5个顶点 → 五边形
   - 6个顶点 → 六边形
   - ≥7个顶点 → 圆形（圆度>0.7且长宽比<1.3）/ 椭圆（轴比>1.15或长宽比>1.25）/ 六边形
...
3. **特征匹配**：
   - 模板相关系数匹配（TM_CCOEFF_NORMED）占55%
   - Hu矩轮廓匹配（平移/缩放/旋转不变性）占25%
   - 宽高比几何特征（区分瘦长数字如"1"）占20%
...
- **输出**：*_shapes.jpg
- **识别算法**：自适应预处理（白色背景用HSV阈值，其他用BGR三通道Canny） + 多边形近似(approxPolyDP, ε=0.015×周长) + 圆度判断 + 拟合椭圆轴比辅助
...
- **算法**：Otsu二值化 → 形态学闭运算 → 轮廓提取 → 模板匹配(55%) + Hu矩匹配(25%) + 几何特征(20%)
...
- **综合评分机制**：即使单个得分不高，模板匹配（55%）+ Hu矩匹配（25%）+ 几何特征（20%）的综合评分仍能正确排序
